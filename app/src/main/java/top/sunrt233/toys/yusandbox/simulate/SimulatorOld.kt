...
class SimulatorOld(private var delta: Float = 0.01f, private val expectedTPS: Float) {
    companion object {
        const val G = 6.67e-11 * 1e12
    }

//    private val simObjectStoreProvider: SimObjectStoreProvider
    ...
    private val maxTimePerTick = ((1 / expectedTPS) * 1e9).toLong()
    ...
    private fun simulate() {
        ...
        while (true) {
            ...
            if (duration < maxTimePerTick) {
                Thread.sleep((maxTimePerTick - duration) / 1e6.toInt())
                tickEndTime = System.nanoTime()
                duration = tickEndTime - tickStartTime
            }
            currentTPS = (1e9 / duration)
    ...
    inner class SimulationInfo {
        ...
        val status: SimulatorStatus
            get() = this@SimulatorOld.status
        val totalStep: Int
            get() = this@SimulatorOld.totalStep
        val delta: Float
            get() = this@SimulatorOld.delta
    }
